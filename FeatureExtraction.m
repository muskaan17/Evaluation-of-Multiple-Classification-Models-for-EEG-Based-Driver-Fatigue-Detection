...
for i = 8:8 %patient numbers
    load(['/MATLAB Drive/EEG_DriverFatigue-master/EEG_DriverFatigue-master/P4/TrainingDataCh',num2str(i)]) %load existing Train dataset
    if Train.data(10,10)~=0 %check if dataset is valid
        ...
        %% 5 Fuzzy WPT feature extraction
        wtlvl = 5;
        ...
        save(['/MATLAB Drive/EEG_DriverFatigue-master/EEG_DriverFatigue-master/P4/FeaturesCh',num2str(i),'.mat'],'F')
...
XX=linspace(0,300,300000);  
%%Plotting
plot(XX,S1);
xlabel('Duration (in Sec)');
ylabel('Amplitude');
title 'Filtered Candidate-5 Data with Channel 8 (Fz)'
%% Fatigue  State
S2=[];
for k=301:600
    S2 = [S2 S(k,:)];
...
fs = 1000;
t = 0:1/fs:300-1/fs;
cwt(S2,"bump",fs) %Fatigue
axis tight
xlabel("Time (Minutes)")
ylabel("Frequency (Hz)")

%%Channel Ploting
Ch = str2double(Channelinfo(2:3,:));
sz=70;
s=scatter(Ch(1,:),Ch(2,:),sz,"filled");
xlabel('Exp')
ylabel('Sim')
text(Ch(1,:),Ch(2,:), Channelinfo(1,:), 'Vert','bottom', 'Horiz','left', 'FontSize',7)
